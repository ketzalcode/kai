...
function missingTargets(screenplay, targets) {
...
function estimateDuration(screenplay) {
  ...
  for (const step of screenplay.steps) {
    ...
    if (step.action === "navigate") total += 1;
    total += step.settle;
  }
  return total;
}
...
function emitDriver(screenplay, targets, options) {
...
function rejects(fn, fragment, label) {
  ...
  try {
    ...
    if (String(error.message).includes(fragment)) console.log(`  ok ${label}`);
      ...
      failures += 1;
      console.log(`  FAIL ${label} (said: ${error.message})`);
    }
  }
}
...
function selfTest() {
  ...
  ok(
    !/metadata=print:file=\$?[A-Za-z]:/.test(driver) && driver.includes("$stats = 'kai-preflight-stats.txt'"),
  ...
  ok(
    driver.indexOf("Wait-Quiet 8000") < driver.indexOf("Click-At $cx $cy; Start-Sleep -Milliseconds 500"),
  ...
  ok(
    estimateDuration(typeOnly) > QUIET_CAP,
  ...
  ok(
    take.pointer.track[0].x === 0.5 && take.pointer.track[0].y === 0.5,
  ...
  ok(parseTake(JSON.stringify({
    ...
    take_id: "t1",
    recording: "raw.mp4",
    capture: { region: [0, 0, 100, 100] },
    steps: [{ id: "a", start: 1, end: 2 }]
...
function main(argv) {
  ...
  if (!screenplayPath) {
    console.error("demo-capture: --screenplay is required");
    return 1;
  }
  if (!existsSync(screenplayPath)) {
    console.error(`demo-capture: screenplay not found: ${screenplayPath}`);
    return 1;
  }
  let screenplay;
  try {
  ...
  if (targetsPath) {
    if (!existsSync(targetsPath)) {
      console.error(`demo-capture: targets not found: ${targetsPath}`);
      return 1;
    }
    try {
...
var invoked = process.argv[1] && process.argv[1].endsWith("demo-capture.mjs");
